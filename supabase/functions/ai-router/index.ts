...
const corsHeaders = (origin?: string | null) => {
  ...
  return {
    ...
    "Access-Control-Allow-Headers": "authorization, Authorization, apikey, x-api-key, content-type, x-client-info, x-requested-with, accept, accept-language, cache-control",
...
const J = (code: number, data: any, origin?: string | null) => new Response(
  JSON.stringify(data), 
  {
    status: code,
    headers: {
...
const tw = async (path: string, init: any = {}) => {
  ...
  try {
    ...
    if (req.method === "POST" && action === "update-design") {
      try {
        ...
        if (!siteId || !design) {
          return J(400, { error: "Missing siteId or design" }, origin);
        }
        ...
        if (colors) {
          ...
          if (primary_color && !hex.test(primary_color)) {
            return J(400, { error: "Invalid primary_color format" }, origin);
          }
          if (secondary_color && !hex.test(secondary_color)) {
            return J(400, { error: "Invalid secondary_color format" }, origin);
          }
          if (background_dark && !hex.test(background_dark)) {
            return J(400, { error: "Invalid background_dark format" }, origin);
          }
        }
        ...
        if (error) {
          return J(500, { error: "Failed to update design" }, origin);
        }
    ...
    if (req.method === "POST" && action === "publish-and-frontpage") {
      try {
        ...
        if (!website_id) {
          return J(400, { error: "Missing website_id" }, origin);
        }
